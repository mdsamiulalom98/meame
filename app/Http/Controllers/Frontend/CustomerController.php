...
class CustomerController extends Controller
{
    ...
    public function complaint_submit(Request $request)
    {
        ...
        $complaint->customer_id = Auth::guard('customer')->user()->id;
        ...
        if ($images) {
            foreach ($images as $key => $image) {
                ...
                $uploadPath = 'public/uploads/complaint/';
                $image->move($uploadPath, $name);
                $imageUrl = $uploadPath . $name;

                $pimage = new ComplaintImage();
                $pimage->complaint_id = $complaint->id;
    ...
    public function signin(Request $request)
    {
        ...
        if ($auth_check) {
            ...
            Toastr::error('Failed', 'Opps! your phone or password wrong');
            return redirect()->back();
        } else {
            Toastr::error('Failed', 'Sorry! You have no account');
            return redirect()->back();
        }
    }
            ...
            if ($exits_customer) {
                ...
                if ($sms_gateway) {
                    ...
                    $data = [
                        ...
                        "type" => 'text',
                        "senderid" => "$sms_gateway->serderid",
                        "msg" => "Dear $request->name!\r\nYour account create successfully, login phone :$request->phone\r\nPassword : $password\r\n.Thank you for using $site_setting->name"
                    ];
                    $ch = curl_init();
                    curl_setopt($ch, CURLOPT_URL, $url);
                    curl_setopt($ch, CURLOPT_POST, 1);
