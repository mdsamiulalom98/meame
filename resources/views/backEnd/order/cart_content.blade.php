    ...
    <tr>
        ...
        <td>
            <div class="qty-cart vcart-qty">
                <div class="quantity">
                    <button class="minus cart_decrement" value="{{ $value->qty }}" data-id="{{ $value->rowId }}"
                        type="button">-</button>
                    <input type="number" value="{{ $value->qty }}" min="1" data-id="{{ $value->rowId }}"
                        class="product_quantity" />
                    <button class="plus cart_increment" value="{{ $value->qty }}" data-id="{{ $value->rowId }}"
                        type="button">+</button>
                </div>
            </div>
        </td>
...
<script>
    ...
    $(".product_quantity").change(function() {
        ...
        $.ajax({
            ...
            url: "{{ route('admin.order.product_quantity') }}",
            dataType: "json",
            success: function(cartinfo) {
                return cart_content() + cart_details();
            },
